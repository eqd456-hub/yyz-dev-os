...
5. Re-run the failed check, affected-area checks, and any broader gates required by the current risk or acceptance contract. Do not repeat unrelated full gates when trustworthy unchanged evidence already covers them.
...
8. Stop the loop when all in-scope true findings are verified fixed, remaining in-scope unverified findings are explicitly resolved or accepted by proper authority, and required gates pass. Record unrelated discoveries for later.
