...
When useful delegation, cross-owner contract/dependency coordination, concurrent workers, reassignment or risk isolation applies, use [Multi-Agent Orchestration](multi-agent-orchestration.md). A registered owner alone does not trigger this protocol: a known small module task uses one compact assignment and direct execution by that module owner. Extend an already required task-local execution card with the assignment contract; do not create a card or second planning system merely to route ownership.
