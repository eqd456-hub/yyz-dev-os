...
Run full Project Recovery when asked to reconstruct project status/history, continue durable work after a Context Handoff or context loss, when project state is stale, missing, or conflicting, or when a high-risk task depends on checkpoints, accepted state, capability evidence, or promotion history.

Do not run full Project Recovery merely because a new chat/model/session begins, chat duration is long, or an ordinary low-risk task changes a local file. For routine work, read project-local instructions, relevant real code/tests/configuration, and current Git status. Escalate only if those facts expose a recovery trigger or continued context use has impaired reliability.
